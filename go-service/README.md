...
Below is a screenshot showing an example of the generated PDF report:

![Student Report PDF Example](docs/images/student-report-example.png)

*Note: The above image shows a sample PDF report generated by the service. The actual content will vary based on the student data retrieved from the backend.*
...
This project is designed to be testable with clear separation of concerns via interfaces (e.g., StudentClient, PDFGenerator).

While extensive unit tests are not included in this submission to keep the focus on core functionality and architecture, the code is structured to easily support testing, especially for:
- Pure utility functions like `formatDOB()`
- Business logic in `StudentReportService` via mocking

Unit tests can be added quickly if required.
